...
public class PackageVersionDaoImpl implements PackageVersionDao {
    ...
    public int insertPackageVersion(PackageVersion packageVersion) throws SQLException {
        String sql = "insert into PackageVersion ( PackageName, PackagePath, HardwareVersion, SystemProgramVersion, ApplicationVersion) values (?, ?, ?, ?, ?)";
        return queryRunner.update(sql, packageVersion.getPackageName(), packageVersion.getPackagePath(), packageVersion.getHardwareVersion(), packageVersion.getSystemProgramVersion(), packageVersion.getApplicationVersion());
    }
